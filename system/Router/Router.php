...
class Router
{
    ...
    public function run(ServerRequestInterface $request,ContainerInterface $container, $callback = null): ResponseInterface
    {
        ...
        if (isset($this->beforeRoutes[$this->requestedMethod])) {
            $this->handle($request, $this->beforeRoutes[$this->requestedMethod]);
        }

        // Handle all routes
        if (isset($this->afterRoutes[$this->requestedMethod])) {
            $response = $this->handle($request, $this->afterRoutes[$this->requestedMethod], true);
        }
     ...
     */
    private function handle(ServerRequestInterface $request, $routes, $quitAfterRun = false): ?ResponseInterface
    {
        ...
        foreach ($routes as $route) {
            ...
            if (preg_match_all('#^' . $route['pattern'] . '$#', $uri, $matches, PREG_OFFSET_CAPTURE)) {
                ...
                $response = $this->invoke($request, $route['fn'], $params);
    ...
    public function getAppRoutes(Router $router, string $path){
        $router = $router;
        include $path;
    }
}
